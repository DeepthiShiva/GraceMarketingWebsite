...
<body id="body">
	...
	try {
		...
		while (orderResultSet.next()) {
			...
			String price = orderResultSet.getString("productPrice");
	...
	<div class="box">
		...
		<div>
			<label style="font-size: 20px"><%=receiverName%></label> <br> <label><%=addressLine1%>,
				<%=addressLine2%></label> <br> <label><%=landmark%></label><br> <label><%=city%></label><br>
			<label><%=pincode%></label><br> <label style="font-size: 20px"><%=phoneNumber%></label>
		</div>
		...
		<div class="status">
			...
			{
				out.println("<label style='color:orange;font-size:15px'>Product needs approval from Admin</label>");
			} else if (status.equals("approved")) {
				out.println(
				"<Label style='color:green;font-size:15px'>Your product has been approved. It will be arriving soon</Label>");
			} else if (status.equals("delivered")) {
			
			out.println("<Label style='color:#E56501;font-size:15px'>This product has been delivered</Label>");
			
			%>

			<a
				href="DeleteOrder.jsp?orderid=<%=orderResultSet.getString("orderid")%>">
				<button type="submit"
					style="background-color: #dedede; color: black; font-weight: bold; display: inline-block; font-size: 11px; letter-spacing: 1px; padding: 14px 35px; text-transform: uppercase; font-weight: 200; border-radius: 0;">Delete</button>
			</a>
			...
			out.println("<Label style='color:red;font-size:15px'>This product has been Rejected by Admin</Label>");
			%>
